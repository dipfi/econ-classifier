...
training_data_agg = training_data[["pubyear_bin", "y", "Journal"]].groupby(["pubyear_bin", "y"]).size().reset_index()
...
if save_plots:
    ...
    plt.savefig(data_path + "/" + plots_tables_folder + "/top5_plot_timeline_proportion_heterodox_in_training")
    plt.close()
...
dtf_weights_top5_tfidf_pivot = dtf_weights_tfidf.pivot(index=["words", "weights"],
                                                  columns=["prediction"],
                                                  values="counts")
...
if save_plots:
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.histplot(data=dtf_weights_top5_tfidf_pivot["weighted_total_count"], kde=True)
...
dtf_weights_top5_tfidf_top20_orthodox = dtf_weights_top5_tfidf_pivot[["word", "weight", "total_count"]][0:20].copy()
dtf_weights_top5_tfidf_top20_heterodox = dtf_weights_top5_tfidf_pivot[["word", "weight", "total_count"]][len(dtf_weights_top5_tfidf_pivot) - 20:len(dtf_weights_top5_tfidf_pivot)].sort_values("weight", ascending=False).copy()

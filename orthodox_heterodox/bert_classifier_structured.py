...
# #NN classifiers
max_length_of_document_vector_list = [900] #np.max([len(i.split()) for i in X_train_series]) #np.quantile([len(i.split()) for i in X_train_series], 0.7) ; suggesion: 8
classifier_loss_function_list = ['sparse_categorical_crossentropy'] #, 'mean_squared_error', 'sparse_categorical_crossentropy', "kl_divergence", 'categorical_hinge'
...
num_epochs_for_embedding_list = [10] #number of epochs to train the word embeddings ; sugegstion: 10(-15) (undersampling training)
...
if journal_split == True:
    ...
    all_journals = dtf[["Journal", label_field]].drop_duplicates().copy().sample(frac = 1)

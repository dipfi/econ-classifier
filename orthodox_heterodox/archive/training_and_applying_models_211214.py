...
if not use_model:
    ...
    ## Save the model to a file or folder in DATA_PATH/models // OVERWRITES PREVIOUS FILES WITH THE SAME NAME
    #### Recommended: False // automatic name is set based on parameter choices
    #### Alternative: e.g. "5TOP5_BERT" // Chose file name according to the settings ("TRAINING", "JOURNALS", "TOP5") and the models used ("TFIDF","W2V", "BERT")
    ...
    save_model = model_file_name = use_training_samples = save_training_samples = False
    ...
    if journal_split:
        ...
        if journal_list != False:
            try:
                ...
                all_journals = all_journals[all_journals[journal_field].isin(journal_list)]

    else:
        all_journals = pd.DataFrame({journal_field: ["randmom"], label_field: ["random"]})
        test_journal = None
        test_label = None

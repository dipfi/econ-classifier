...
#balancing main dtf
dtf_samequality = dtf[dtf[label_field]=="samequality"]
dtf_heterodox = dtf[dtf[label_field]=="heterodox"]
...
for index, all_test in all_journals.iterrows():
    ...
    for i in random.sample(range(1, len(y_train)), 10):
    ...
    time_loop = start_loop-end_loop
    len_dtf = len(dtf_test)
    logger.info(f"For Loop {count} finished. N_test = {len_dtf}, time = {time_loop}")

results.to_csv(str(data_path + "/" + output_file_name + ".csv"), index = False)

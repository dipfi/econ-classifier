...
results_file_name = "REPRODUCTION_1Model_Selection_TFIDF"
...
if current_model == "tfidf":
    ...
    tfidf_classifier_list = ["LogisticRegression", "naive_bayes", "RandomForestClassifier", "GradientBoostingClassifier"]  # ["naive_bayes", "LogisticRegression", "RandomForestClassifier","GradientBoostingClassifier", "SVC"]
    ## choose a classifier
...
if current_model == "w2v":
    ...
    if train_new:
        num_epochs_for_embedding_list = [5, 10, 15]
        ## choose number of epochs to train the word embeddings
        #### Recommendation: [5]
        #### Alternative: [10,15]

        window_size_list = [4, 8, 12]
    ...
    if not embedding_only:
        num_epochs_for_classification_list = [5, 10, 15]
